...
:::tip
For more detailed information on how Glow’s lending pools operate and dynamically adjust rates, see our [Lending Pool Products](https://docs.jetprotocol.io/jet-protocol/protocol/jet-products/pooled-variable-lending-interest-rates-design) section, which outlines the mechanics of variable interest rates and risk management.
:::

3. **Leverage Swaps**: This feature enable users to borrow additional funds to trade with higher amounts than they actually own, maximizing their potential gains (or losses). Users can choose different strategies like going long or short on an asset, depending on their market outlook.
...
1. The user deposits USDC as collateral in their Glow margin account.
2. Using the Leverage Swaps feature, they borrow SOL and immediately swap it for sSOL.
3. The user holds sSOL, which grows in value over time as staking rewards accumulate, effectively earning yield in a delta-neutral position.
4. Since the user borrows SOL (but holds sSOL), they are protected against price swings in SOL, maintaining a stable position that still benefits from staking rewards.
