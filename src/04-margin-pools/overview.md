...
4. **Risk Management**:
   The system manages liquidity through utilization-based rates, collateral requirements, and a safety buffer that caps pool utilization at 95%.

   If a margin account has residual, unrecoverable debt after liquidation, the protocol may invoke a governance-controlled bad-debt write-off flow. In that flow, loan notes are written off and losses are reflected through the pool's lender exchange rate, which may reduce lender redemption value on a pro-rata basis for that pool.

   Protocol fees accrued for an asset pool may act as a partial backstop over time, but are not guaranteed to fully cover losses.
