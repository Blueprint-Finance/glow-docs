...
1. **Interest Generation**: Deposits in a margin account are automatically transferred to margin pools, where they may begin accruing variable interest.
...
Glow’s system continuously monitors this health level, issuing alerts to users and providing real-time updates to help them monitor exposure.
...
- **Maximum Leverage Buffer**: The maximum borrowing allowed leaves the health level at **6.0%**, allowing users to approach the maximum leverage permitted under current protocol settings.
...
- **Optimize Trade Execution**: By leveraging adapters, users can access optimized routes through aggregators like Jupiter on Solana, seeking efficient trade routing across DEXs, subject to external routing logic and availability.
...
- **Risk Segmentation**: Users can create accounts dedicated to different strategies, intended to isolate positions at the account level under the current protocol design.
